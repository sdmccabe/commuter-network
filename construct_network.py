...
def main():
    df = pd.read_excel("data/raw/table3.xlsx",
                    skiprows=7,
                    header=None,
                    names=SCHEMA.keys(),
                    dtype=SCHEMA,
    ...
    town_gazetteer = pd.read_csv("data/raw/2019_Gaz_cousubs_national.txt",
                           sep="\t",
                           dtype={"GEOID": str},
                           )
    ...
    county_gazetteer = pd.read_csv("data/raw/2019_Gaz_counties_national.txt",
                           sep="\t",
                           dtype={"GEOID": str},
                           )
    ...
    df['source_fips'] = df.apply(lambda row:
                                 construct_fips(row.source_state_fips_code,
                                                row.source_county_fips_code,
                                                row.source_mcd_fips_code),
                                 axis=1)

    df['target_fips'] = df.apply(lambda row:
                                 construct_fips(row.target_state_fips_code,
    ...
    df = df.merge(source_gazetteer,
                  how="left",
                  on="source_fips",
    )
    df = df.merge(target_gazetteer,
                  how="left",
                  on="target_fips",
    )
    ...
    # about locations.
    target_attr_df = (df.set_index('target_fips')
                    .loc[:, ['target_state_name',
                             'target_county_name',
                             'target_mcd_name']]
                    .rename({
    ...
    source_attr_df = (df.set_index('source_fips')
                    .loc[:, ['target_state_name',
                             'target_county_name',
                             'target_mcd_name']]
                    .rename({
